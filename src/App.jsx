...
const STRIDEPlatform = () => { const [currentView, setCurrentView] = useState('landing'); // landing, discover, dashboard, resources, offer-recommendation, case-studies const [messages, setMessages] = useState([ { role: 'assistant', content: "Hey. I'm going to ask you some questions to figure out if systematising your business makes sense right now. This isn't sales stuff—it's diagnosis. So I'm going to be direct. What type of service business do you run?" } ]); const [input, setInput] = useState(''); const [loading, setLoading] = useState(false); const [discoveryStage, setDiscoveryStage] = useState(0); // 0-5 const messagesEndRef = useRef(null); 

// User profile that builds as they answer questions const [userProfile, setUserProfile] = useState({ industry: null, teamSize: null, mainPainPoint: null, hasTriedAutomation: null, canImplement: null, email: null }); 

const [recommendedOffer, setRecommendedOffer] = useState(null); 

// Industry-specific information const industries = { contractor: { name: 'Contractor / Facilities / Security', emoji: '🏗️', painPoints: 'Estimate writing (3-4 hours per estimate), scope documentation scattered, compliance requirements, team scheduling', roiTimeline: '2-3 weeks', timeSaved: '12-14 hours/week', caseStudies: [ { name: 'Sarah', location: 'Austin, TX', before: '3.5 hours per estimate', after: 'Now 1 hour. Winning 2-3 extra clients/month.' } ] }, photographer: { name: 'Wedding/Graduation Photographer', emoji: '📸', painPoints: 'Switching between 3+ systems for one client question, duplicate client data entry, gallery management scattered', roiTimeline: '4-6 weeks', timeSaved: '9 hours/week', caseStudies: [ { name: 'Jennifer', location: 'Portland, OR', before: 'Client questions: 3 hours/week', after: 'Now 30 minutes. Freed time for portfolio shoots = 3 more bookings.' } ] }, courier: { name: 'Courier / Delivery', emoji: '📦', painPoints: 'Route changes = phone call chaos, driver communication slow, missed deliveries from confusion', roiTimeline: '2-4 weeks', timeSaved: '6-8 hours/week', caseStudies: [ { name: 'David', location: 'Chicago, IL', before: '30 min/day on route communication', after: 'Now 5 min/day. 23% fewer no-shows.' } ] }, cleaner: { name: 'Residential/Commercial Cleaning', emoji: '🧹', painPoints: 'Team callouts destroy margins, route coverage scrambling, last-minute cancellations losing revenue', roiTimeline: '2-3 months', timeSaved: '8-10 hours/week', caseStudies: [ { name: 'Jennifer', location: 'Houston, TX', before: 'Covering 20% of callouts', after: 'Now 70%. Extra $1,200/month from prevented cancellations.' } ] }, healthcare: { name: 'Healthcare Agency / Home Health', emoji: '🏥', painPoints: 'Documentation flow slow, real-time visibility missing, billing delayed 10 days because documentation lags', roiTimeline: 'Immediate', timeSaved: 'Cash flow improves by 10 days ($60K/month)', caseStudies: [ { name: 'Jennifer', location: 'Atlanta, GA', before: 'Billing on day 15', after: 'Now day 5. $60K monthly cash flow improvement.' } ] }, coach: { name: 'Wellness/Business Coach', emoji: '🧠', painPoints: 'Client context scattered, follow-ups inconsistent, forgetting key details, client retention lower than possible', roiTimeline: '6-8 weeks', timeSaved: '5-7 hours/week', caseStudies: [ { name: 'Michelle', location: 'New York, NY', before: 'Digging for context before sessions', after: 'Instant access. Sessions deeper. 12% retention improvement.' } ] } }; 

const scrollToBottom = () => { messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }); }; 

useEffect(() => { scrollToBottom(); }, [messages]); 

const handleMessageSend = async (e) => { e.preventDefault(); if (!input.trim()) return; 
...
// LANDING PAGE if (currentView === 'landing') { return ( <div style={{ background: '#F7F6F3', minHeight: '100vh', fontFamily: 'IBM Plex Sans, sans-serif' }}> {/* Navigation */} <div style={{ background: '#2C3E50', color: '#F7F6F3', padding: '16px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #D97634' }}> <div style={{ fontSize: '18px', fontWeight: '600' }}>LSSB <div style={{ display: 'flex', gap: '20px', fontSize: '13px' }}> <button onClick={() => setCurrentView('landing')} style={{ background: 'none', border: 'none', color: '#F7F6F3', cursor: 'pointer' }}>Home <button onClick={() => setCurrentView('discover')} style={{ background: 'none', border: 'none', color: '#F7F6F3', cursor: 'pointer' }}>Start Discovery <button onClick={() => setCurrentView('case-studies')} style={{ background: 'none', border: 'none', color: '#F7F6F3', cursor: 'pointer' }}>Results  
...
// DISCOVERY VIEW if (currentView === 'discover') { return ( <div style={{ display: 'flex', height: '100vh', background: '#F7F6F3', fontFamily: 'IBM Plex Sans, sans-serif' }}> {/* Sidebar */} <div style={{ width: '300px', background: '#2C3E50', color: '#F7F6F3', padding: '32px 24px', borderRight: '1px solid #D97634', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}> <h3 style={{ fontSize: '13px', fontWeight: '600', color: '#D97634', margin: '0 0 24px 0' }}> Discovery Progress  
...
// CASE STUDIES VIEW if (currentView === 'case-studies') { return ( <div style={{ background: '#F7F6F3', minHeight: '100vh', fontFamily: 'IBM Plex Sans, sans-serif' }}> {/* Navigation */} <div style={{ background: '#2C3E50', color: '#F7F6F3', padding: '16px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #D97634' }}> <div style={{ fontSize: '18px', fontWeight: '600' }}>LSSB <div style={{ display: 'flex', gap: '20px', fontSize: '13px' }}> <button onClick={() => setCurrentView('landing')} style={{ background: 'none', border: 'none', color: '#F7F6F3', cursor: 'pointer' }}>Home <button onClick={() => setCurrentView('discover')} style={{ background: 'none', border: 'none', color: '#F7F6F3', cursor: 'pointer' }}>Start Discovery  
...
// OLD:
API_BASE: process.env.REACT_APP_API_BASE || 'https://api.stridei.app',

// NEW:
API_BASE: process.env.REACT_APP_API_BASE || 'https://stride-backend-b5qqs6qeoa-ew.a.run.app',
